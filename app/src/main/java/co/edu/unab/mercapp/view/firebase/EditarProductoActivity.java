...
public class EditarProductoActivity extends AppCompatActivity {
    ...
    private void loadProducto() {
        productos.document(idDocumento).get().addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
            @Override
    ...
    public void eliminar(View view){
        productos.document(idDocumento).delete().addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
            public void onSuccess(Void unused) {
                EditarProductoActivity.this.setResult(RESULT_OK);
                finish();
            }
        }).addOnFailureListener(new OnFailureListener() {
            @Override
            public void onFailure(@NonNull Exception e) {
                Toast.makeText(getApplicationContext(),"ERRO al eliminar",Toast.LENGTH_LONG);
            }
        });
    }
    public void guardar(View view){
        productos.document(idDocumento).delete();
    }
}